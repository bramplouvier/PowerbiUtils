...
$outPath = "$PSScriptRoot/out/PowerbiUtils"
...
Push-Location "$PSScriptRoot/PowerbiUtils.Engine"
try
{
    dotnet publish
}
...
Push-Location "$PSScriptRoot/PowerbiUtils.Cmdlets"
try
{
...
Copy-Item -Path "$PSScriptRoot/PowerbiUtils.psd1" -Destination $outPath
Get-ChildItem -Path "$PSScriptRoot/PowerbiUtils.Engine/bin/$Configuration/netstandard2.0/publish" |
    Where-Object { $_.Extension -in '.dll','.pdb' } |
    ForEach-Object { [void]$commonFiles.Add($_.Name); Copy-Item -LiteralPath $_.FullName -Destination $commonPath }
Get-ChildItem -Path "$PSScriptRoot/PowerbiUtils.Cmdlets/bin/$Configuration/$netCore/publish" |
...
Get-ChildItem -Path "$PSScriptRoot/PowerbiUtils.Cmdlets/bin/$Configuration/$netFramework/publish" |
    Where-Object { $_.Extension -in '.dll','.pdb' -and -not $commonFiles.Contains($_.Name) } |
